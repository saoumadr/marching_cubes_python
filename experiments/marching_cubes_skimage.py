"""
marching_cubes_skimage.py

Original Marching Cubes (Lorensen & Cline, 1987) surface extraction using scikit-image.

WARNING:
This implementation DOES NOT solve topological ambiguities.
It is included for comparison and educational purposes.
...
def extract_surface_original_mc(mask, spacing, iso_value=0.5):
    """
...
def main():
    ...
    for i, lbl in enumerate(labels):
        ...
        t0 = time.time()
        mesh = extract_surface_original_mc(mask, spacing, ISO_VALUE)
        t1 = time.time()
    ...
    plotter.add_axes()
    plotter.show(title="Marching Cubes (Lorensen–Cline)")
